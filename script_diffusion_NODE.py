...
parser.add_argument(
    "--gamma_epochs", type=int, default=20, help="Scheduling gamma decay epochs"
)
parser.add_argument("--final_gamma", type=float, default=0.0)
parser.add_argument("--final_lr", type=float, default=1e-6)
parser.add_argument("--decay_steps", type=int, default=750)
...
if autodecoder:
    if not DINO:
        ...
        dataset_train = DiffusionTrajDatasetTorch(
            path, max_step, indices=(0, num_samples), paramed=paramed
        )
    dataset_train_full = DiffusionTrajDatasetTorch(
        path, max_step * 2, indices=(0, 16), paramed=paramed
    )
    dataset_validation = DiffusionTrajDatasetTorch(
        path, max_step * 2, indices=(100, 116), paramed=paramed
    )
    subdataset_train = DiffusionTrajDatasetTorch(
        path, max_step * 2, indices=(0, 16), paramed=paramed
    )
else:
...
if not DINO and node_training_mode != NodeTrainingModeEnum.ZERO:
    if args.pinn:
        evolve_fn = automatic_diffusion_evolve_builder(2.0, False, False, False)
    else:
        evolve_fn = diffusion_residual
else:
    evolve_fn = None
