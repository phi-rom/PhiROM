...
from PHIROM.pde.navier_turbulence_irregular import *
...
parser.add_argument(
    "--dataset", type=str, default="ns_turbulence_irregular1024_64x64_ins=5"
)
...
if autodecoder:
    if not DINO:
        dataset_train = IrregularDecayingTurbulenceDatasetTorch(
            path, max_step, indices=(0, num_samples), paramed=paramed
        )
    else:
        dataset_train = IrregularDecayingTurbulenceTrajDatasetTorch(
            path,
            max_step,
    ...
    dataset_validation = IrregularDecayingTurbulenceTrajDatasetTorch(
        path, max_step * 2, indices=(256, 256 + 16), paramed=paramed
    )
    subdataset_train = IrregularDecayingTurbulenceTrajDatasetTorch(
...
if arch == "mlp":
    ...
    hyperparams["input_scale"] = max(dataset_train.length)
...
if not DINO and node_training_mode != NodeTrainingModeEnum.ZERO:
    evolve_fn = cfd_residual_builder(dataset_train.cfd_grid)
else:
    evolve_fn = None
