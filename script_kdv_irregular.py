...
from PHIROM.pde.kdv_2d_irreg import *
...
if autodecoder:
    if not DINO:
        dataset_train = IrregularKdV2dDatasetTorch(
            path, max_step, indices=(0, num_samples), paramed=paramed
        )
    else:
        dataset_train = IrregularKdV2dTrajectoryDatasetTroch(
            path, max_step, indices=(0, num_samples), paramed=paramed
        )
    dataset_validation = IrregularKdV2dTrajectoryDatasetTroch(
        path, max_step * 2, indices=(512, 512 + 16), paramed=paramed
    )
    subdataset_train = IrregularKdV2dTrajectoryDatasetTroch(
