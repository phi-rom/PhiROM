...
parser.add_argument(
    "--node_training_mode",
    type=str,
    default=NodeTrainingModeEnum.JACOBIAN_PSI,
    choices=[
...
if autodecoder:
    if node_training_mode in [
        NodeTrainingModeEnum.JACOBIAN_PSI,
        NodeTrainingModeEnum.JACOBIAN_INVERSE,
    ]:
        dataset_train = BurgersDatasetTorch(path, max_step, indices=(0, num_samples))
    else:
        dataset_train = BurgersTrajectoryDatasetTroch(
            path, max_step, indices=(0, num_samples)
        )
    dataset_train_full = BurgersTrajectoryDatasetTroch(
        path, 200, indices=(0, num_samples)
    )
    dataset_validation = BurgersTrajectoryDatasetTroch(
        path, 200, indices=(num_samples, num_samples + 8)
    )
    subdataset_train = BurgersTrajectoryDatasetTroch(
        path, 200, indices=(0, num_samples)
    )
else:
...
callbacks = [
    CheckpointCallback(path_checkpoint, name, hyperparams, True, 500),
    NODEUnrollingEvaluationCallback(
        dataset_validation,
        max_step,
        200,
...
key, subkey = jax.random.split(key)
if (
    node_training_mode == NodeTrainingModeEnum.JACOBIAN_PSI
    or node_training_mode == NodeTrainingModeEnum.JACOBIAN_INVERSE
):
    if not args.pinn:
